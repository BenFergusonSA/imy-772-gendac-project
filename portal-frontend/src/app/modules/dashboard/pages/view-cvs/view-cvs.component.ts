      ...
      if (xhr.readyState === 4) {
          let temp = JSON.parse(xhr.responseText)["Items"].map((item: { email: any; firstName: any; id: any; phoneNumber: any; surname: any; uuid: any; }) => {
            return {
              ...
              uuid: item.uuid.S,
              pdfURL: "https://gendac-cvs80138-dev.s3.amazonaws.com/public/"+item.uuid.S+".pdf",
              skills: ["Loading..."],
              education: ["Loading..."],
              experience: "Loading...",
              hasSkills: false,
        ...
        if (xhr.readyState === 4) {
          let filteredSkills = JSON.parse(xhr.responseText).map((applicant: any[]) => {
            return {
              ...
              pdfURL: "https://gendac-cvs80138-dev.s3.amazonaws.com/public/"+applicant[0].Item.uuid.S+".pdf",
